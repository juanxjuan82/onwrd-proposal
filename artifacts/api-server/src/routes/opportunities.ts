...
router.post("/opportunities", async (req, res) => {
  ...
  try {
    const score = scoreOpportunity(title, description, category ?? "General");
...
// ── Get opportunity with requirements + bid score ──────────────────────────
router.get("/opportunities/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({ ...tender, requirements, bidScore: bidScore ?? null });
  } catch (err) {
...
Extract all distinct, actionable requirements. Be thorough — include submission format requirements, eligibility criteria, deliverable specs, timeline constraints, and any compliance items.`,
    ...
    await db.delete(tenderRequirementsTable).where(eq(tenderRequirementsTable.tenderId, id));
...
function scoreOpportunity(title: string, description: string, category: string): number {
  const MARKETING_KEYWORDS = [
    "marketing", "advertising", "branding", "brand", "communications",
    "public relations", "digital", "social media", "campaign", "creative",
    "content", "graphic design", "design", "video", "website", "web design",
    "seo", "promotion", "outreach", "tourism marketing", "event", "media",
  ];
